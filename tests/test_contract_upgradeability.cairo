use attendsys::contracts::AttenSysOrg::{
    AttenSysOrg, IAttenSysOrgDispatcher, IAttenSysOrgDispatcherTrait, IUpgradeableDispatcher,
    IUpgradeableDispatcherTrait,
};
use snforge_std::{
    ContractClassTrait, EventSpyAssertionsTrait, declare, spy_events, start_cheat_caller_address,
...
fn test_upgrade_attensys_org() {
    ...
    spy.assert_emitted(@array![// Add any expected events from the upgrade process
    ]);
}
