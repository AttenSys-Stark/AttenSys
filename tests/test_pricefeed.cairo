...
use attendsys::contracts::oracle::PriceFeed::IPriceFeedExampleABIDispatcher;
use attendsys::contracts::oracle::PriceFeed::IPriceFeedExampleABIDispatcherTrait;
...
fn test_pricefeed() {
    ...
    let pricefeed = IPriceFeedExampleABIDispatcher {
        contract_address: pricefeed_address,
    };
...
fn test_pricefeed_work_with_course_creation() {
    ...
    assert(attensys_course_contract.get_price_of_strk_usd() == price_of_strk_usd, 'Price should match oracle value');
  ...
  // create a course with price 25 USD
    ...
    attensys_course_contract.create_course(owner, true, base_uri.clone(), name.clone(), symbol.clone(), base_uri_2.clone(), price);
    ...
    assert(courses[0].price == @(price * price_of_strk_usd), 'Price should match oracle value');

    // create a course with price 0 USD
    let price_2: u128 = 0; //0 USD
    start_cheat_caller_address(course_contract, owner_two);
    attensys_course_contract.create_course(owner_two, true, base_uri.clone(), name.clone(), symbol.clone(), base_uri_2.clone(), price_2);
    stop_cheat_caller_address(course_contract);

    // get_all_courses_info
    let courses = attensys_course_contract.get_all_courses_info();
    assert(courses.len() == 2, 'Course should be created');
    assert(courses[1].price == @(price_2 * price_of_strk_usd), 'Price should match oracle value');
}
